...
def process_video(frames, prompt, min_presence=0.25, export_path=None, fps_out=10):
    ...
    # Luego, por frame: el frame original (jpg) + las máscaras POR CAPA
    # (blanca donde está el objeto). El cliente las decodifica una vez y las
    # compone en un solo lienzo, así controla color/visibilidad/caja sin desfase.
    sset = set(stable)
    for fi, tracked in enumerate(per_frame):
        ...
        jb = io.BytesIO()
        frames[fi].save(jb, format="JPEG", quality=75)
